...
// Exécution de la requête SQL avec la condition améliorée
$stmt = $bdd->query('SELECT * FROM clients WHERE ID_STRIPE IS NULL OR TRIM(ID_STRIPE) = ""');
if (!$stmt) {
...
if ($stmt->rowCount() === 0) {
    ...
    echo $stmt->rowCount() . ' clients trouvés sans ID Stripe.<br>';
}
